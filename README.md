...
"A implementação anterior funcionou perfeitamente. Agora, preciso melhorar o design e o layout da página 'Início', acredito que seja controlada pelo home.html do backend. O objetivo é organizar o posicionamento dos elementos criando uma hierarquia visual mais clara.

Requisitos da UI/UX:

Ação Principal (Destaque): Crie uma área central ou de destaque com um botão grande 'Iniciar Fluxo'. Esse fluxo representa o processo completo (importar CSV -> seguir etapas -> resultado final).

Ações Secundárias: Mantenha a funcionalidade atual que permite ir direto para etapas específicas. No entanto, o design deve tratar essas opções como secundárias. Diminua o tamanho desses botões e posicione-os de forma que não tirem a atenção do botão principal.

O que eu preciso:

Uma breve sugestão de como organizar esse layout (ex: usando cards, colunas ou painéis).
